...
class Database:
    ...
    def delete_leave_request(self, request_id):
        ...
        print(request_id)
        cursor.execute(
            "DELETE FROM tbl_leave_request WHERE request_id = ?", (request_id,)
        )
        conn.commit()
        conn.close()
    ...
    def update_employee(self, emp_id, data):
        ...
        if data["password"]:
            ...
            cursor.execute(
                "SELECT password FROM tbl_employee WHERE emp_id = ?", (emp_id,)
            )
    ...
    def delete_project(self, project_id):
        ...
        # Check for associated tasks
        cursor.execute(
            "SELECT COUNT(*) FROM tbl_task WHERE project_id = ?", (project_id,)
        )
        task_count = cursor.fetchone()[0]
    ...
    def get_all_tasks_with_details_paginated(
        self, page, page_size, project_filter="", status_filter="", employee_filter=""
    ):
        conn = self.get_connection()
        cursor = conn.cursor()
    ...
    def add_leave_type(self, leave_type):
        try:
            with self.get_connection() as c:
                c.execute(
                    "INSERT INTO tbl_leave_type (leave_type) VALUES (?)", (leave_type,)
                )
    ...
    def get_leave_types(self):
        with self.get_connection() as c:
            return c.execute(
                "SELECT leave_type_id, leave_type FROM tbl_leave_type ORDER BY leave_type"
            ).fetchall()
    ...
    def get_leave_status(self, req_id):
        with self.get_connection() as c:
            result = c.execute(
                "SELECT status FROM tbl_leave_request WHERE request_id=?", (req_id,)
            ).fetchone()
    ...
    def add_expense_type(self, etype):
        with self.get_connection() as c:
            c.execute(
                "INSERT OR IGNORE INTO tbl_expense_type (expense_type) VALUES (?)",
                (etype,),
            )

    def get_leave_requests_with_advanced_filters(
        self,
        employee_id=None,
        leave_type_id=None,
        status=None,
    ...
    def get_expense_types(self):
        with self.get_connection() as c:
            return c.execute(
                "SELECT expense_type_id, expense_type FROM tbl_expense_type ORDER BY expense_type"
            ).fetchall()
    ...
    def update_expense_type(self, et_id, new_type):
        with self.get_connection() as c:
            c.execute(
                "UPDATE tbl_expense_type SET expense_type = ? WHERE expense_type_id = ?",
                (new_type, et_id),
            )
    ...
    def expense_type_exists(self, etype, exclude_id=None):
        with self.get_connection() as c:
            if exclude_id:
                ...
                result = c.execute(
                    "SELECT 1 FROM tbl_expense_type WHERE expense_type = ?", (etype,)
                ).fetchone()
    ...
    def update_expense_status(
        self, exp_id, status, approver_comments="", manager_id=None, approved_by=None
    ):
        with self.get_connection() as c:
            if status == "approved":
    ...
    def get_employee_profile(self, emp_id):
        with self.get_connection() as conn:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM TblEmployeeProfile WHERE EmployeeId = ?", (emp_id,)
            )
            row = cursor.fetchone()
    ...
    def update_employee_password_and_emgcontact(
        self, emp_id, new_password, emg_contact
    ):
        if new_password or new_password is not None:
            with self.get_connection() as conn:
                ...
                cursor.execute(
                    "UPDATE tbl_employee SET password = ? WHERE emp_id = ?",
                    (hashed, emp_id),
                )
            ...
            with self.get_connection() as conn:
                ...
                hashed = self.hash_password(new_password)
                cursor.execute(
                    "UPDATE TblEmployeeProfile SET EmgContact = ? WHERE EmployeeID = ?",
                    (emg_contact, emp_id),
                )
    ...
    def soft_delete_wiki_page(self, wiki_id):
        with self.get_connection() as conn:
            conn.execute(
                "UPDATE TblWikiPage SET RowStatus=1 WHERE WikiId=?", (wiki_id,)
            )
        # ---------- Wiki Views CRUD ----------
    ...
    def add_policy_to_db(
        self, policy_name, filepath, filename, original_filename, file_size
    ):
    ...
    def get_employee_anniversaries(self, filter_type="anniversary", days_limit=7):
        """Get employees with upcoming anniversaries or birthdays within specified days"""
        conn = self.get_connection()
        cursor = conn.cursor()
