...
# def log_error_to_json(error_data):
#     """
#     Append error details to errors.json in the main folder.
#     """
#     # Check if file exists; if not, create with empty list
...
#     """
#     Global exception handler for all Flask routes.
#     """
...
#         "description": "An unexpected error occurred. Check errors.json for details.",
#         "code": 500
#     }), 500
...
UPLOAD_FOLDER = "static/bngImg"
INVOICE_FOLDER = "static/invoices"
WIKI_CAT_FOLDER = "static/wiki_categories"
DATABASE_PATH = "project_tracking.db"
...
os.makedirs(WIKI_CAT_FOLDER, exist_ok=True)
...
def view_employees():
    ...
    return render_template(
        "view_employees.html", employees=employees, status_filter=status_filter
    )
...
def edit_task(task_id):
    ...
    employees = db.get_employees()
    return render_template(
        "edit_task.html", task=task, projects=projects, employees=employees
    )
...
def add_task():
    ...
    today = date.today().isoformat()  # format: 'YYYY-MM-DD'
    return render_template(
        "add_task.html", projects=projects, employees=employees, today=today
    )
...
def employee_profile_view():
    ...
    if request.method == "POST":
        new_password = request.form.get("password", "").strip()
        new_emg = request.form.get("EmgContact", "").strip()
        alert_msg = db.update_employee_password_and_emgcontact(
            emp_id, new_password, new_emg
        )
    ...
    return render_template(
        "my_profile.html", employee=employee, profile=profile, alert_msg=alert_msg
    )
...
def view_task_details(task_id):
    ...
    return render_template(
        "view_task_details.html", task=task, details=details, project=project
    )
...
def edit_task_detail(detail_id):
    ...
    if not detail or not db.verify_task_detail_owner(detail_id, session["user_id"]):
        flash(
            "Task update not found or you do not have permission to edit it.", "error"
        )
        return redirect(url_for("employee_dashboard"))
...
def get_task_details(task_id):
    try:
        if "user_id" not in session:
            logger.warning(
                f"Unauthorized access attempt to task details for task_id {task_id}"
            )
            return jsonify({"error": "Unauthorized"}), 401
            ...
            details_list.append(
                {
                    "detail_id": detail[0],
                    "desc": detail[1],
                    "inserted_date": detail[2]
                    if detail[2]
                    else None,  # Handle null dates
                    "status": detail[3],
                }
            )

        logger.debug(
            f"Successfully fetched {len(details_list)} task details for task_id {task_id}"
        )
        return jsonify(details_list)

    except Exception as e:
        logger.error(
            f"Error in get_task_details for task_id {task_id}: {str(e)}", exc_info=True
        )
        return jsonify({"error": "Internal server error", "message": str(e)}), 500
...
def admin_leave_types():
    ...
    if request.method == "POST":
        ...
        if not leave_type:
            ...
            success = db.update_leave_type(int(lt_id), leave_type)
            popup_message = (
                "Leave type updated successfully." if success else "Update failed."
            )
            popup_type = "success" if success else "error"
        else:  # Add
            success, message = db.add_leave_type(leave_type)
...
def employee_celebrations():
    ...
    # Get celebrations data within 7 days only
    celebrations = db.get_employee_anniversaries(filter_type, days_limit=7)

    # Filter today's celebrations from the already filtered list
    today_celebrations = [c for c in celebrations if c[4] == 0]
...
def expense():
    ...
    types_ = db.get_expense_types()
    employees = (
        db.get_employees(status_filter="active")
        if session["emp_type"] == "admin"
        else []
    )
    return render_template("expense.html", types=types_, employees=employees)
...
def existing_expenses():
    ...
    if session["emp_type"] == "admin":
        ...
        where_clause = "WHERE " + " AND ".join(filters) if filters else ""
        total = db.count_expenses(where_clause, tuple(params))
        expenses = db.get_expenses_paginated(
            where_clause, tuple(params), per_page, offset
        )
    ...
    for e in expenses:
        ...
        try:
            if e[IDX_INSERTED_DATE]:
                utc_time = datetime.fromisoformat(e[IDX_INSERTED_DATE])
                e[IDX_INSERTED_DATE] = utc_time.astimezone(ist).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
        except Exception:
            pass
        ...
        try:
            if e[IDX_APPROVED_DATE]:
                utc_time = datetime.fromisoformat(e[IDX_APPROVED_DATE])
                e[IDX_APPROVED_DATE] = utc_time.astimezone(ist).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            else:
                e[IDX_APPROVED_DATE] = "Not set"
...
def delete_job(id):
    conn = get_db_connection()
    job = conn.execute(
        "SELECT BannerImg FROM TblCareers WHERE CareerId = ?", (id,)
    ).fetchone()

    if job and job["BannerImg"]:
        img_path = os.path.join(
            current_app.root_path, "static", "bngImg", job["BannerImg"]
        )
        if os.path.exists(img_path):
            os.remove(img_path)
...
def edit_asset(asset_id):
    ...
    conn = get_db_connection()
    asset = conn.execute(
        "SELECT * FROM TblAssets WHERE AssetId = ?", (asset_id,)
    ).fetchone()
...
def allocate_asset():
    ...
    conn = get_db_connection()
    assets = conn.execute(
        "SELECT * FROM TblAssets WHERE Status = 'Available'"
    ).fetchall()
    employees = conn.execute(
        "SELECT emp_id, first_name, last_name FROM tbl_employee WHERE status = 'active'"
    ).fetchall()
    ...
    if request.method == "POST":
        ...
        conn.execute(
            "UPDATE TblAssets SET Status = 'Allocated' WHERE AssetId = ?",
            (request.form["asset_id"],),
        )
        conn.commit()
        conn.close()
...
def edit_allocation(alloc_id):
    ...
    if request.method == "POST":
        conn.execute(
            "UPDATE TblAllocateAssets SET Status = 'Returned' WHERE AllocatedId = ?",
            (alloc_id,),
        )
        conn.execute(
            "UPDATE TblAssets SET Status = 'Available' WHERE AssetId = ?",
            (allocation["AssetId"],),
        )
        conn.commit()
        conn.close()
...
def add_policy():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # Save to database
            db.add_policy_to_db(
                policy_name, filepath, unique_filename, original_filename, file_size
            )
...
def view_policy(policy_id):
    ...
    if os.path.exists(filepath):
        ...
        return send_from_directory(
            directory, filename, as_attachment=True, download_name=original_name
        )
    else:
        flash("Policy file not found", "error")
        return redirect(url_for("list_policies"))
...
def employee_view_policy(policy_id):
    ...
    if os.path.exists(filepath):
        ...
        filename = os.path.basename(filepath)
        return send_from_directory(
            directory, filename, as_attachment=False, mimetype="application/pdf"
        )
    else:
        flash("Policy file not found", "error")
        return redirect(url_for("employee_policies"))
